...
from ArtBay.forms import FilterProduceForm, AddProduceForm, BuyProduceForm, RestockProduceForm
from ArtBay.models import Produce as ProduceModel, ProduceOrder
from ArtBay.queries import insert_produce, get_produce_by_pk, Sell, \
    insert_sell, get_all_produce_by_Artist, get_produce_by_filters, insert_produce_order, update_sell, \
...
@login_required
def add_produce():
    form = AddProduceForm(data=dict(Artist_pk=current_user.pk))
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            sell = Sell(dict(Artist_pk=current_user.pk, produce_pk=new_produce_pk, available=True))
            insert_sell(sell)
...
def your_produce():
    ...
    if request.method == 'GET':
        produce = get_all_produce_by_Artist(current_user.pk)
    if request.method == 'POST':
        produce = get_produce_by_filters(category=request.form.get('category'),
                                         item=request.form.get('item'),
                                         variety=request.form.get('variety'),
